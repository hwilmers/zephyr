...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_manufacturer)
{
	...
	LOG_INF("Manufacturer: %s", log_strdup(modem.minfo.mdm_manufacturer));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_model)
{
	...
	LOG_INF("Model: %s", log_strdup(modem.minfo.mdm_model));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_revision)
{
	...
	LOG_INF("Revision: %s", log_strdup(modem.minfo.mdm_revision));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_imei)
{
	...
	LOG_INF("IMEI: %s", log_strdup(modem.minfo.mdm_imei));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_imsi)
{
	...
	LOG_INF("IMSI: %s", log_strdup(modem.minfo.mdm_imsi));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_iccid)
{
	...
	LOG_INF("ICCID: %s", log_strdup(modem.minfo.mdm_iccid));
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_rssi_cesq)
{
	...
	modem.context.data_rssi = modem.minfo.mdm_rssi;
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_rssi_csq)
{
	...
	if (argc > 0) {
		...
		if (modem.minfo.mdm_rssi == 0 && rssi == GSM_RSSI_MAXVAL) {
			rssi = 0;
		}

		modem.minfo.mdm_rssi = rssi;
		modem.context.data_rssi = rssi;
		LOG_DBG("RSSI: %d", rssi);
	}
...
#if defined(CONFIG_MODEM_GSM_ENABLE_CESQ_RSSI)
static const struct setup_cmd read_rssi_cmd =
...
static const struct setup_cmd setup_cmds[] = {
	...
	/* hang up */
#if defined(MODEM_GSM_FLOWCONTROL_OFF)
		...
		if (ret < 0) {
			...
			if (ret < 0) {
				...
				LOG_INF("AT channel %d connected to %s",
					DLCI_AT, log_strdup(gsm->at_dev->name));
			}
		}

		modem_cmd_handler_tx_unlock(&gsm->context.cmd_handler);
		if (gsm->state != GSM_PPP_STATE_ERROR) {
			(void)gsm_work_reschedule(&gsm->rssi_work_handle, K_NO_WAIT);
		}
	}
...
static void mux_setup(struct k_work *work)
{
	...
	struct gsm_modem *gsm = CONTAINER_OF(dwork, struct gsm_modem,
					     gsm_configure_work);
#if defined(CONFIG_MODEM_GSM_CONFIG)
	const struct device *const uart = device_get_binding(gsm_config.uart_name);
#else
	const struct device *const uart = device_get_binding(CONFIG_MODEM_GSM_UART_NAME);
#endif
	int ret;
	...
	case GSM_PPP_STATE_DONE:
		...
		LOG_INF("PPP channel %d connected to %s",
			DLCI_PPP, log_strdup(gsm->ppp_dev->name));
...
void gsm_ppp_start(const struct device *dev)
{
	...
	ret = modem_iface_uart_init_dev(&gsm->context.iface, uart);
	if (ret < 0) {
...
DEVICE_DEFINE(gsm_ppp, GSM_MODEM_DEVICE_NAME, gsm_init, NULL, &modem, NULL,
